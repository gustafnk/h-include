...
window.HInclude.HIncludeElement = window.HIncludeElement = (function() {
  ...
  function showContent(element, req){
    ...
    if (req.status === 200 || req.status === 304) {
      ...
      if(element.getAttribute('dispatchOnError')) {
        document.dispatchEvent(new CustomEvent('h-include.reqError', {detail: {req}}));
      }
    }
  ...
  var include = function(element, url, includeCallback) {
    if(element.getAttribute('when') && !element.conditionalInclusion.call(element)) {
      throw new Error('one or multiple conditions not met')
    }
  ...
  /* the when attribute points to global function that holds an array of conditions.
  each item in the array is an object that look like this:
  {type: conditionType, value: functionOrMediaQuery }

  example:
  window.fragmentConditions = [
  {type: 'media', value: "screen and (max-width: 600px)"},
  {type: 'function', value: "fnName"}
  ]

  All conditions of this function needs to return true for the when to pass
  */
  proto.conditionalInclusion = function() {
    var includeConditions = this.getAttribute('when');
    if(window[includeConditions]) {
      return window[includeConditions]().every(function(condition) {
        if(window.matchMedia && condition.type === 'media') {
          return window.matchMedia(condition.value).matches;
        } 
        if(window[condition.value]) {
          return window[condition.value]();
        } else {
          throw new Error('Condition: ' + condition.value + ' not found.')
        }
        return false;
      })
    } else {
      throw new Error('Conditional functions not found');
    }
  }
