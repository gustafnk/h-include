...
window.HInclude.HIncludeElement = window.HIncludeElement = (function() {
  ...
  var reqErrorAltPredicate = function(element, req) {
    var alternativeUrl = element.getAttribute('alt');
    return req.status !== 200 && req.status !== 304 && alternativeUrl && !requestAlt;
  };

  var include = function(element, includeCallback) {
    var url = element.getAttribute('src');
    var alternativeUrl = element.getAttribute('when-false-src');
    ...
    // Use alternative url on request failure or failed conditional inclusion
    if(requestAlt) {
      url = alternativeUrl;
    } 
    ...
    req.onreadystatechange = function () {
      if (req.readyState === 4) {
        // Check predicate for alternative src if a request error occurs
        if(reqErrorAltPredicate(element, req)) {
          requestAlt = true;
