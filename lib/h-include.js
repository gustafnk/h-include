...
window.HInclude.HIncludeElement = window.HIncludeElement = (function() {
  ...
  function showContent(element, req){
    ...
    if (req.status === 200 || req.status === 304) {
      ...
      if (config('checkRecursion', element)) {
  ...
  var config = function(key, element) {
    var configuration = window.HIncludeConfig;
    if(!configuration) {
      return undefined;
    }

    var configKey = element.getAttribute('config-key');
    if(configKey) {
      if(!configuration.configKeys) {
        throw new Error('A config-key attribute was specified but no HIncludeConfig.configKeys was found');
      }
      else if(!configuration.configKeys[configKey]){
        throw new Error('A config-key attribute requires a matching key within the HincludeConfig object');
      } else {
        configuration = configuration.configKeys[configKey];
      }
    } 

    var value = key === 'debug' ? configuration : configuration[key];
  ...
  proto.onEnd = function(req) {
    ...
    var configuration = config('debug', this);
    if(configuration.debug === true) {
      this.setAttribute('data-config', JSON.stringify(configuration));
    }
  ...
  proto.connectedCallback = function() {
    var mode = config('mode', this) || 'buffered';
    ...
    if (mode === 'async') {
      ...
      var timeout = config('timeout', this) || 2500;
