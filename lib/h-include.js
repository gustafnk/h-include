...
window.HInclude.HIncludeElement = window.HIncludeElement = (function() {
  ...
  proto.connectedCallback = function() {
    var elementConfig = this.getAttribute('configKey');
    if(elementConfig && !config) {
      throw new Error('A configKey attribute requires the HincludeConfig to be available on the window object');
    } else if(elementConfig && !config[elementConfig]) {
      throw new Error('A configKey attribute requires a matching key within the HincludeConfig object');
    } else {
      config = config[elementConfig];
    }
