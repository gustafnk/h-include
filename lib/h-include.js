...
window.HInclude.HIncludeElement = window.HIncludeElement = (function() {
  ...
  var include = function(element, includeCallback) {
    ...
    // Skip conditionalInclusion if performing an altSrcInclude pass
    if((!altSrcInclude && !element.conditionalInclusion.call(element))) {
      if(whenFalseUrl) {
        url = whenFalseUrl;
      } 
      else if(!whenFalseUrl && altUrl) {
        // Use alt url if failed conditional inclusion lacks a when-false-src
        altSrcInclude = true;
      }
      else {
        // Exit if there is no altUrl / a media query is not fulfilled
        return;
      }
    }

    // Use alt url if a request error occured or if a conditional inclusion failed and
    // there was no when-false-src present
    if(altSrcInclude) {
      url = altUrl;
    }
    ...
    req.onreadystatechange = function () {
      if (req.readyState === 4) {
        // Predicate for an alternative src should a request error occur
        if(req.status !== 200 && req.status !== 304 && altUrl && !altSrcInclude) {
          altSrcInclude = true;
          // Run altSrcInclude pass with alternative url 
  ...
  proto.connectedCallback = function() {
    ...
    if (mode === 'async') {
      ...
      var timeout = config && config.timeout || 100;
